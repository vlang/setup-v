...
async function run() {
    try {
        ...
        // Versioned cache entries require an explicit version even when architecture is set.
        if (arch && !version) {
            core.warning('`architecture` is provided but `version` is missing. V will be installed for the requested architecture from the default branch, and versioned caching will be skipped. To select a specific version and enable versioned caching, provide `version`.');
        }
        if (!arch) {
            arch = os.arch();
        }
